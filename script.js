...
const hardQuestions = [
    ...
    },
    {
        ...
        options: ["The Terrible", "The Mighty", "The Chiefest and Greatest of Calamities", "The Golden"],
        correct: 2,
    ...
    },
    {
        ...
        options: ["Öldürüldüler", "Kayıp gittiler", "Doğu'ya göç ettiler", "Bilinmiyor"],
        correct: 3,
        category: "Ent Tarihi",
        difficulty: "hard"
    },
    ...
    },
    {
        ...
        options: ["Galadriel'in oğlu", "Fëanor'un torunu", "Gil-galad'ın kardeşi", "Elrond'un babası"],
        correct: 1,
...
function saveQuizAttempt(score, totalQuestions, duration) {
    ...
    localStorage.setItem(STORAGE_KEYS.QUIZ_HISTORY, JSON.stringify(limitedHistory));
...
function updateStatsDisplay(stats = null) {
    ...
    document.getElementById('bestScore').textContent = stats.bestScore ? `${stats.bestScore.percentage}%` : '-';
    document.getElementById('averageScore').textContent = stats.totalAttempts > 0 ? `${stats.averagePercentage}%` : '-';
    ...
    quizContainer.innerHTML = `
        <div class="quiz-header fade-in">
            ...
            <div class="progress-container">
                ...
                <div class="difficulty-indicator">${difficultyTitle[currentQuiz.difficulty] || '🧙‍♂️ Wizard Seviyesi'}</div>
            </div>
        ...
        <div class="next-button-container" id="nextButtonContainer" style="display: none;">
            <button class="next-button" onclick="nextQuestion()">
                <span class="button-gradient" id="nextButtonText">➡️ Sonraki Soru</span>
            </button>
        </div>
    `;
}
...
function selectAnswer(answerIndex) {
    ...
    if (currentQuiz.currentQuestion + 1 < currentQuiz.questions.length) {
        nextButtonText.textContent = '➡️ Sonraki Soru';
    } else {
        nextButtonText.textContent = '🎯 Sonuçları Gör';
    }
    
    nextButtonContainer.style.display = 'block';
    nextButtonContainer.classList.add('slide-up');
...
function confirmBackToHome() {
    if (confirm("Quiz'den çıkmak istediğinizden emin misiniz? İlerlemeniz kaybolacak.")) {
        showHomeScreen();
    }
}
    ...
    resultsContainer.innerHTML = `
        ...
        <div class="score-circle fade-in">
            <div class="score-circle-inner">
                <div class="score-display">${attempt.score}/${attempt.totalQuestions}</div>
        ...
        <div class="stats-container slide-up">
            ...
            <div class="stat-item">
                <div class="stat-number">${attempt.totalQuestions - attempt.score}</div>
        ...
        <div class="social-share-container slide-up">
            ...
            <div class="share-buttons">
                <button class="share-button twitter" onclick="shareOnTwitter(${attempt.percentage}, '${characterSuggestion.character}', '${attempt.difficulty}')">
                    <span class="share-icon">🐦</span>
                    <span class="share-text">Twitter'da Paylaş</span>
                </button>
                <button class="share-button copy" onclick="copyResultToClipboard(${attempt.percentage}, '${characterSuggestion.character}', '${attempt.difficulty}')">
                    <span class="share-icon">📋</span>
        ...
        <button class="start-button slide-up" onclick="startQuiz('${attempt.difficulty || 'mixed'}')">
...
function getResultMessage(percentage) {
    if (percentage >= 90) {
        return {
            ...
            message: "Mükemmel! Gandalf bile sizden bilgi alabilir. Orta Dünya'nın en derin sırlarını biliyorsunuz!"
        };
    } else if (percentage >= 70) {
        return {
            emoji: "⚔️",
            title: "Ranger Seviyesi!",
            message: "Harika performans! Aragorn gibi Orta Dünya'nın koruyucususunuz. Bilginiz etkileyici!"
        };
    } else if (percentage >= 50) {
        return {
            emoji: "🍃",
            title: "Hobbit Bilgisi!",
            message: "İyi iş çıkardınız! Shire'da rahat bir yaşam sürebilirsiniz. Biraz daha çalışmaya ne dersiniz?"
        };
    } else if (percentage >= 30) {
        return {
            emoji: "🌱",
            title: "Yeni Başlayan!",
            message: "Orta Dünya yolculuğunuz daha yeni başlıyor. Daha fazla kitap okuma zamanı!"
        };
    } else {
        return {
            emoji: "🤔",
            title: "Serüveniniz Başlasın!",
            message: "Görünüşe göre Orta Dünya'ya yeni adım atıyorsunuz. Kitapları okumaya başlamanın tam zamanı!"
        };
    }
}
...
function getCharacterSuggestion(percentage) {
    if (percentage >= 90) {
        return {
            ...
            description: "Bilginiz ve hikmetiniz Orta Dünya'da efsanevi. İstalar arasında bile özel bir yeriniz var!"
        };
    } else if (percentage >= 80) {
        return {
            character: "Elrond",
            description: "Bin yıllık bilgi birikiminiz var. Rivendell'in efendisi olarak herkesi yönlendirebilirsiniz."
        };
    } else if (percentage >= 70) {
        return {
            character: "Aragorn",
            description: "Lider ruhuyla donanmış, bilgili ve cesur bir Rangers'sınız. Gondor'un kralı olmaya hazırsınız!"
        };
    } else if (percentage >= 60) {
        return {
            character: "Legolas",
            description: "Elf zekanızla parlıyorsunız. Mirkwood'un prensesi olarak ait olduğunuz yeri biliyorsunuz."
        };
    } else if (percentage >= 50) {
        return {
            character: "Samwise Gamgee",
            description: "Sadık ve cesur! Her ne kadar her şeyi bilmeseniz de, doğru zamanda doğru yerde olursunuz."
        };
    } else if (percentage >= 40) {
        return {
            character: "Frodo Baggins",
            description: "Maceraya atılmaya hazır genç bir Hobbit! Yolculuğunuz daha yeni başlıyor."
        };
    } else if (percentage >= 30) {
        return {
            character: "Merry Brandybuck",
            description: "Meraklı ve öğrenmeye açık! Shire'ın sınırlarını keşfetmeye hazırsınız."
        };
    } else {
        return {
            character: "Pippin Took",
            description: "Genç ve deneyimsiz ama cesaretli! Orta Dünya maceralarınız daha yeni başlıyor."
        };
    }
}
...
function createHistoryHTML() {
    ...
    let historyHTML = `
        <div class="quiz-header fade-in">
            ...
            ${history.length > 0 ? '<button class="back-button" onclick="confirmClearHistory()">Temizle</button>' : ''}
        </div>
    `;
            ...
            historyHTML += `
                <div class="result-message slide-up" style="margin-bottom: 20px; border-left: 4px solid ${scoreColor};">
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
                        <div>
                            <div style="color: #D4AF37; font-weight: 600;">${formatDate(attempt.date)}</div>
                            ...
                            <div style="color: #B0B0B0; font-size: 0.8rem; margin-top: 5px;">
                                ${difficultyIcon[attempt.difficulty] || '🧙‍♂️'} ${difficultyName[attempt.difficulty] || 'Wizard'}
                            </div>
                        </div>
                    ...
                    <div style="display: flex; justify-content: space-between; font-size: 0.9rem;">
                        ...
                        <span>Yanlış: <strong>${attempt.totalQuestions - attempt.score}</strong></span>
                        <span>Süre: <strong>${Math.round(attempt.duration)}s</strong></span>
                    </div>
                </div>
            `;
        });
...
function confirmClearHistory() {
    if (confirm('Tüm quiz geçmişinizi silmek istediğinizden emin misiniz? Bu işlem geri alınamaz.')) {
        clearQuizHistory();
        createHistoryHTML();
    }
}
...
function shareOnTwitter(percentage, character, difficulty) {
    ...
    const text = `Lord of the Rings Quiz'de %${percentage} puan aldım! Benim karakterim: ${character} 🧙‍♂️ ${difficultyText[difficulty] || 'Wizard'} seviyesinde! Sen de dene:`;
...
function copyResultToClipboard(percentage, character, difficulty) {
    ...
    const text = `Lord of the Rings Quiz'de %${percentage} puan aldım! Benim karakterim: ${character} 🧙‍♂️ ${difficultyText[difficulty] || 'Wizard'} seviyesinde! Sen de dene: ${window.location.href}`;
